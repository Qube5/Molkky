...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    gray_cp = (gray_img <=lower_thresh).astype(int)+(gray_img >= upper_thresh).astype(int)
    print(gray_cp[0])
...
def edge_detect_naive(gray_img):
    ...
    # 5. Return G
    gray_temp = cv2.GaussianBlur(gray_s, (5, 5), 0)

    Kx = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]])
    Ky = np.array([[-1, -2, -1], [0, 0, 0], [1, 2, 1]])
    Gx = ndimage.convolve(gray_temp, Kx, mode='constant', cval=0.0)
    Gy = ndimage.convolve(gray_temp, Ky, mode='constant', cval=0.0)
...
def segment_image(img): 
    ...
    # binary = threshold_segment_naive(to_grayscale(img), 80, 150).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    test_img_2 = read_image(IMG_DIR + '/lego.jpg')
    test_img_2_gray = read_image(IMG_DIR + '/lego.jpg', grayscale=True)
    test_img_3 = read_image(IMG_DIR + '/rgb_test.jpg')


    # uncomment the test you want to run
    # it will plot the image and also save it
    lower = input("Input lower threshold")
    upper = input("Input upper threshold")
    test_thresh_naive(test_img, lower, upper)
    # test_edge_naive(test_img_2_gray)
    # test_edge_canny(test_img_2_gray)
    # test_cluster(test_img_2, 5)
